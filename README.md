...
- 默认使用 `https://aicode.cat` 作为接口地址，并从本机 Codex 配置读取 API 密钥
...
- 默认使用 `https://aicode.cat`，并读取本机 Codex 配置中的 API 密钥
...
更推荐把密钥放在环境变量里，不要把真实 API 密钥发进聊天：
...
--metadata FILE        保存不含 b64_json 的响应元数据
--timeout SECONDS      curl 超时时间，默认 300
--overwrite            允许覆盖已有输出文件
--dry-run              试运行，打印脱敏请求信息，不调用接口
```
