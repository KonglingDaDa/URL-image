...
1. 克隆 `https://github.com/KonglingDaDa/URL-image`，将 `skill_src/image-curl` 复制到 `~/.codex/skills/image-curl`（或 `$CODEX_HOME/skills/image-curl`）。
2. 为 `scripts/*.sh` 添加可执行权限。
3. 复制 `local.env.example` 为 `local.env`，将用户提供的生图专用 Key 写入 `IMAGE_CURL_API_KEY=`。
4. 执行 `chmod 600 local.env`。
5. 运行 `generate_image.sh --dry-run` 验证配置，再执行一次真实生图测试。
