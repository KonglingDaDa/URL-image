...
本 skill **兼容 macOS 与 Windows**，但前提是必须在 **Bash 环境** 中运行脚本，不能直接在 PowerShell / CMD 中执行。
...
|---|---|
| `bash` | 执行本 skill 脚本 |
| `curl` | 请求 `/v1/images/generations` 与 `/v1/images/edits` |
| `python3` | 读取配置、处理 JSON、解码 `b64_json` |
...
任一命令缺失时，不要改 skill 业务逻辑；先帮用户补齐依赖，再重试。

### 平台差异

- **macOS**：默认终端即可，直接按安装步骤执行。
- **Windows**：必须使用 **Git Bash** 或 **WSL**。若当前是 PowerShell 且 `bash` 不可用，切换到 Git Bash / WSL 后再安装和测试。
- 全程只在**同一个 Bash 环境**里操作，避免 Windows 路径（如 `E:\...`）与 WSL 路径（如 `/mnt/e/...`）混用。

### 环境异常时 Agent 怎么处理

按下面顺序处理，**不要擅自把脚本改成 PowerShell**：

1. **找不到 bash**：引导用户在 Windows 上启用 WSL，或安装 Git for Windows 后使用 Git Bash。
2. **找不到 curl / python3**：在当前 Bash 环境中安装缺失工具；macOS 可用 Homebrew，WSL 可用 `apt`。
3. **Permission denied**：为 `scripts/*.sh` 执行 `chmod +x`。
4. **$'\r': command not found` 或脚本解析异常**：说明 `.sh` 被改成 CRLF，改回 LF 后重试。
5. **路径找不到**：统一改用当前 Bash 环境能访问的绝对路径，重新传 `--output` 或 `--image`。
6. **API Key 已配置但仍报错**：检查 `local.env` 是否在 `~/.codex/skills/image-curl/local.env`，且由脚本自动加载。

只有用户明确要求“必须原生 PowerShell、不能走 Bash”时，才考虑单独做 PowerShell 版本；默认不要修改现有 `.sh` 实现。

## 安装与 Key 配置（Agent 必读）

当用户要求安装本 skill 并配置生图专用 Key 时，按以下步骤执行：

1. 确认当前是 Bash 环境，并检查 `bash`、`curl`、`python3` 均可用。
2. 克隆 `https://github.com/KonglingDaDa/URL-image`，将 `skill_src/image-curl` 复制到 `~/.codex/skills/image-curl`（或 `$CODEX_HOME/skills/image-curl`）。
3. 为 `scripts/*.sh` 添加可执行权限。
4. 复制 `local.env.example` 为 `local.env`，将用户提供的生图专用 Key 写入 `IMAGE_CURL_API_KEY=`。
5. 执行 `chmod 600 local.env`。
6. 运行 `generate_image.sh --dry-run` 验证配置，再执行一次真实生图测试；Windows 用户也在 Git Bash / WSL 中完成上述步骤。
...
3. 文生图：运行本技能的 `scripts/generate_image.sh`。脚本组装 JSON，默认调用 `curl -X POST https://aicode.cat/v1/images/generations`，暂存原始响应，解码返回的 `data[].b64_json`，并写入图片文件。
4. 图生图编辑：运行本技能的 `scripts/edit_image.sh`。脚本以多部分（multipart）表单默认调用 `curl -X POST https://aicode.cat/v1/images/edits`，并重复附加 `image[]=@<文件>` 字段。
