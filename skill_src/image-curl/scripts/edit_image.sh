...
usage() {
  cat <<'USAGE'
用法：
  edit_image.sh --image 文件 --prompt 文本 --output 文件 [选项]
  edit_image.sh --image 文件 --prompt-file 文件 --output 文件 [选项]
...
[[ "${#images[@]}" -gt 0 ]] || die "至少需要一个 --image 参数。"
...
for image in "${images[@]}"; do
  [[ -n "$image" ]] || die "--image 不能为空。"
  [[ -f "$image" ]] || die "未找到图片文件：$image"
  [[ -s "$image" ]] || die "图片文件为空：$image"
